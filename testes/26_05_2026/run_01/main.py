...
def ket_brickwall_ansatz(layers, theta_np, qubits):
    """
    Constrói o ansatz brickwall no Ket.
    """
    for l in range(layers):
        for j in range(3):
            RX(float(theta_np[l, j, 0]),qubits[j])
            RY(float(theta_np[l, j, 1]),qubits[j])
            RZ(float(theta_np[l, j, 2]),qubits[j])

        # Brickwall local:
        CNOT(qubits[0], qubits[1])
        CNOT(qubits[1], qubits[2])

def local_unitary_from_ket(theta_sub):
    """
    Extrai a matriz 8x8 do circuito simulado no Ket para o PyTorch.
    """
    if torch.is_tensor(theta_sub):
        theta_np = theta_sub.detach().numpy()
    else:
        theta_np = theta_sub

    layers = theta_np.shape[0]

    # Wrapper para o dump_matrix do Ket
    def ansatz_wrapper(qubits):
        ket_brickwall_ansatz(layers, theta_np, qubits)

    # Extração da matrix unitária em Ket
    U_numpy = np.array(qulib.dump_matrix(ansatz_wrapper, 3))

    # Conversão para torch
    U_torch = torch.tensor(U_numpy, dtype=torch.complex128)
...
def train_once(lam, A, chi, seed=0, layers=5, epochs=epochs, lr=0.01, verbose=False):
    ...
    G0_re_init = 0.10 * np.random.randn(sub_dim, chi)
    G0_im_init = 0.10 * np.random.randn(sub_dim, chi)
    G1_re_init = 0.10 * np.random.randn(chi, sub_dim, chi)
    G1_im_init = 0.10 * np.random.randn(chi, sub_dim, chi)
    G2_re_init = 0.10 * np.random.randn(chi, sub_dim)
    G2_im_init = 0.10 * np.random.randn(chi, sub_dim)

    # Achatamento do parametros em um vetor 1D para o SciPy
    initial_params = np.concatenate([
        theta_init.flatten(),
        G0_re_init.flatten(), G0_im_init.flatten(),
        G1_re_init.flatten(), G1_im_init.flatten(),
        G2_re_init.flatten(), G2_im_init.flatten()
    ])

    loss_hist = []

    def unpack_params(flat_params):
        """Desempacota o vetor 1D de volta para os tensores do modelo."""
        idx = 0

        size = theta_init.size
        theta_np = flat_params[idx : idx+size].reshape(theta_init.shape)
        idx += size

        size = G0_re_init.size
        G0_re_np = flat_params[idx : idx+size].reshape(G0_re_init.shape)
        idx += size

        size = G0_im_init.size
        G0_im_np = flat_params[idx : idx+size].reshape(G0_im_init.shape)
        idx += size

        size = G1_re_init.size
        G1_re_np = flat_params[idx : idx+size].reshape(G1_re_init.shape)
        idx += size
        
        size = G1_im_init.size
        G1_im_np = flat_params[idx : idx+size].reshape(G1_im_init.shape)
        idx += size
        
        size = G2_re_init.size
        G2_re_np = flat_params[idx : idx+size].reshape(G2_re_init.shape)
        idx += size
        
        size = G2_im_init.size
        G2_im_np = flat_params[idx : idx+size].reshape(G2_im_init.shape)

        return (
            torch.tensor(theta_np, dtype=torch.float64),
            torch.tensor(G0_re_np, dtype=torch.float64),
            torch.tensor(G0_im_np, dtype=torch.float64),
            torch.tensor(G1_re_np, dtype=torch.float64),
            torch.tensor(G1_im_np, dtype=torch.float64),
            torch.tensor(G2_re_np, dtype=torch.float64),
            torch.tensor(G2_im_np, dtype=torch.float64)
        )
    
    def objective(flat_params):
        """Função custo que o SciPy tentará minimizar."""
        theta, G0_re, G0_im, G1_re, G1_im, G2_re, G2_im = unpack_params(flat_params)
        loss = expected_energy(theta, G0_re, G0_im, G1_re, G1_im, G2_re, G2_im, energies)
        val = float(loss.item())

        loss_hist.append(val)
        if verbose and len(loss_hist) % 20 == 0:
            print(f"Iteração {len(loss_hist):04d} | loss = {val:.6f}")
...
def print_example_circuit(layers=5):
    """
    Mostra o subcircuito local usando a função de desenho nativa do Ket.
    """
    print("\n=== Subcircuito local Ket ===")
    theta_dummy = np.zeros((layers, 3, 3))

    def dummy_ansatz(qubits):
        ket_brickwall_ansatz(layers, theta_dummy, qubits)
        
    try:
        qulib.draw(dummy_ansatz, 3)
    except Exception as e:
        print("Gráfico omitido no terminal, consulte notebook. Erro:", e)
